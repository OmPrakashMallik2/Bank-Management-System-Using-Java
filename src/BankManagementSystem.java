...
public class BankManagementSystem {
    ...
    public static ResultSet getDetailsTransactions(Connection connection, int accountNumber, int pinNumber) {
        ResultSet resultSet = null;
        try {
            String query = "SELECT * FROM transactions WHERE account_number = " + accountNumber + " AND pin = " + pinNumber + ";";
            Statement statement = connection.createStatement();
            resultSet = statement.executeQuery(query);
        } catch (Exception e) {
            System.out.println(e.getStackTrace());
    ...
    public static void addDetailsTransactions(long account_number, int pin, String statement_text, double amount, double balance, Connection connection) {
        int resultSet = 0;
        try {
            String query = "INSERT INTO transactions (account_number, pin,email, statement_text, amount, balance, transaction_date) VALUES ("+account_number+", "+pin+", 'email not fount', '"+statement_text+"', "+ amount+", "+balance+", CURRENT_DATE);";
            Statement statement = connection.createStatement();
                ...
                while (true) {
                    ...
                    if (choice2 == 1) {
                        ...
                        if (rs11.next() && amount > rs11.getDouble(8)) {
                            ...
                            // add transaction row
                            addDetailsTransactions(accountNumber, pinNumber, "Debited from account", amount, rs12.getDouble(8), connection);
                        }
                    }
                    else if (choice2 == 2) {
                        ...
                        if (rs41.next()){
                            ...
                            // add transaction row
                            addDetailsTransactions(accountNumber, pinNumber, "Credited to account", amount, rs42.getDouble(8), connection);
                        }
                    }
                    else if (choice2 == 5) {
